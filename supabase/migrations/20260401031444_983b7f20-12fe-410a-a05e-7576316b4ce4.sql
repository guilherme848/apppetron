...
-- Add source column to targets
ALTER TABLE petron_sales_funnel_targets
  ADD COLUMN source text NOT NULL DEFAULT 'inbound';

-- Add source column to actuals
ALTER TABLE petron_sales_funnel_actuals
  ADD COLUMN source text NOT NULL DEFAULT 'inbound';

-- Drop and recreate view with source column
...
WITH client_sales AS (
  SELECT
    date_trunc('month', accounts.start_date::timestamp)::date AS month,
    count(*)::integer AS sales_count,
    COALESCE(sum(accounts.monthly_value), 0)::numeric(15,2) AS total_revenue,
    CASE WHEN count(*) > 0 THEN sum(accounts.monthly_value) / count(*)::numeric ELSE 0::numeric END::numeric(15,2) AS avg_ticket
  FROM accounts
  WHERE accounts.start_date IS NOT NULL AND accounts.deleted_at IS NULL
  GROUP BY date_trunc('month', accounts.start_date::timestamp)::date
),
monthly_data AS (
  SELECT
...
-- Update unique constraints to include source
ALTER TABLE petron_sales_funnel_targets DROP CONSTRAINT IF EXISTS petron_sales_funnel_targets_month_key;
ALTER TABLE petron_sales_funnel_actuals DROP CONSTRAINT IF EXISTS petron_sales_funnel_actuals_month_key;

-- Add new unique constraints with source
CREATE UNIQUE INDEX IF NOT EXISTS petron_sales_funnel_targets_month_source_idx ON petron_sales_funnel_targets (month, source);
CREATE UNIQUE INDEX IF NOT EXISTS petron_sales_funnel_actuals_month_source_idx ON petron_sales_funnel_actuals (month, source);
