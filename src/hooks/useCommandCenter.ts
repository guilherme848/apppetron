...
export function useCommandCenter() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [
      ...
      supabase.from('cs_client_onboarding').select('*, accounts:client_id(name)').limit(500),
      supabase.from('cs_meetings').select('*, accounts:client_id(name), team_members:responsible_member_id(name)').order('meeting_date', { ascending: false }).limit(500),
      supabase.from('cs_nps_responses').select('*, accounts:client_id(name)').order('created_at', { ascending: false }).limit(500),
      supabase.from('cs_playbooks').select('*, accounts:client_id(name), team_members:responsible_member_id(name)').limit(500),
      supabase.from('traffic_playbook_tasks').select('id, client_id, status, due_date').limit(500),
      supabase.from('cs_churn_events').select('*, accounts:client_id(name, niche_id, service_id, state), team_members:owner_member_id(name)').order('created_at', { ascending: false }).limit(500),
      supabase.from('cs_cancellations').select('*, accounts:client_id(name)').order('created_at', { ascending: false }).limit(500),
